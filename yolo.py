...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            #   对预测框进行得分筛选
            #---------------------------------------------------------#
            top_index = batch_detections[:,4] * batch_detections[:,5] > self.confidence     # 需要检测框的置信度大于 self.confidence
        ...
        for i, c in enumerate(top_label):
            predicted_class = self.class_names[c]   # 框的类
            score = top_conf[i]                     # 框的得分

            top, left, bottom, right = boxes[i]     #左上右下的点
            top = top - 5
            left = left - 5
            bottom = bottom + 5
            right = right + 5
